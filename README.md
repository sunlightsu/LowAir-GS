...
> **Demo-01：摄影测量三维模型与无人机实时数据虚实融合显示系统**（Qt6 + OpenGL + UDP JSON）

| 初始场景（两个摄影测量模型已加载） | 无人机实时飞行（157 轨迹点） |
|:---:|:---:|
| ![初始场景](docs/images/demo01_initial_scene.png) | ![无人机实时飞行](docs/images/demo01_uav_live.png) |

| 完整椭圆轨迹（358 轨迹点，虚实融合效果） |
|:---:|
| ![完整轨迹](docs/images/demo01_full_trajectory.png) |

**图示说明：**
- **灰白色网格体**：Stanford Bunny 真实激光扫描模型（69,451 三角面），由 Assimp 加载，OpenGL 光照渲染
- **绿色地形网格**：泊松曲面重建地形（9,769 三角面），摄影测量典型输出
- **橙色折线**：无人机历史飞行轨迹（最多 1000 点），20 Hz 实时更新
- **左侧面板**：深色主题 GCS 状态面板，实时显示 UAV ID、位置、姿态、电量、轨迹点数
